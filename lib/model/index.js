...
var SubscriptionFormModel = function () {
	...
	for (i = 0; i < INTEREST_FIELDS.length; ++i) {
		this.values["interest_" + INTEREST_FIELDS[i]] = "";
	}
	for (i = 0; i < SKILL_FIELDS.length; ++i) {
		this.values["skill_" + SKILL_FIELDS[i]] = "";
	}
	for (i = 0; i < PRACTICE_FIELDS.length; ++i) {
		this.values["practice_" + PRACTICE_FIELDS[i]] = "";
	}
	for (i = 0; i < LANG_SKILL_FIELDS.length; ++i) {
		this.values["lang_skill_" + LANG_SKILL_FIELDS[i]] = "";
	}
...
var INTEREST_FIELDS = ["db", "gui", "crypt", "embeded", "theory", "ai", "games",
		"maintenance", "perf", "team", "rt", "security", "social", "sports",
		"web"];
var SKILL_FIELDS = ["rdb", "nosql", "big_data", "gui", "crypt", "embeded",
...
SubscriptionFormModel.prototype.getInterestRank = function (name) {
	return parseInt(this.values["interest_" + name], 16);
};
...
SubscriptionFormModel.prototype.getSkillLevel = function (name) {
	return parseInt(this.values["skill_" + name], 16);
};
...
SubscriptionFormModel.prototype.getPracticeLevel = function (name) {
	return parseInt(this.values["practice_" + name], 16);
};
 ...
 */
SubscriptionFormModel.prototype.getLangSkillLevel = function (name) {
